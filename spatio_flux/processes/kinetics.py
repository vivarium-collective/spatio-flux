...
def get_kinetics_process_from_registry(
    ...
    return {
        ...
        "config": model_config,
        "inputs": {
            "substrates": {mol_id: build_path(path, mol_id, 'concentrations', i, j) for mol_id in mol_ids},
            "biomass": build_path(path, biomass_id, 'concentrations', i, j)
        },
        "outputs": {
            "substrates": {mol_id: build_path(path, mol_id, 'counts', i, j) for mol_id in mol_ids},
            "biomass": build_path(path, biomass_id, 'counts', i, j)
        }
    }
