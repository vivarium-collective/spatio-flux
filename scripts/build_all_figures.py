...
def main() -> None:
    ...
    if not args.force:
        edited = [n for n in build if fp.hand_edited(n, manifest)]
        if edited:
            for n in edited:
    ...
    # 1. seed git views -> .pbg so the headless render is deterministic from git.
    _run(sync + ["--seed"], step="seed: git views -> .pbg (deterministic render)")

    # 2. render stale loom panels (one job list to the jobless renderer).
    ...
    # 4. stitch affected studies (each study's final step).
    ...
    # 5. record the manifest + log a figure-build event.
    ...
    print(f"\n[build_all_figures] rebuilt {len(build)} node(s); {len(nodes) - len(build)} left untouched.")
